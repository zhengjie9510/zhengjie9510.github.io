...
你有没有过这种经历：想让 AI 帮你看看某个网站最新发了什么文章，结果它试着去访问，直接被反爬虫机制挡了回来——要么返回一堆乱码，要么干脆什么都拿不到。

这不是 AI 本身的问题。很多网站对自动化请求的检测越来越严格，AI 发起的网络查询经常被当作爬虫拦截。但这里面有一个有意思的突破口：**大部分网站同时会提供一个 RSS 订阅链接——一种专门给机器读的、结构化的信息接口，而且几乎不会被反爬机制拦截。**

所以我写了一份 RSS 阅读技能——让 AI 不直接爬网页，而是通过 RSS 这个"官方通道"来获取信息。现在只要跟它说一句"帮我看看最近三天 AI 和编程相关的博客有什么更新"，几十秒之后，它就按源整理好、按时间排好序、连摘要都截取得干干净净，摆在面前。

**更爽的是下一步：扫一眼摘要，看到感兴趣的，直接说一句"帮我把第 3 篇打开"——浏览器自动弹出来，直接跳到原文。** 整个过程你只负责做筛选和决策，跑腿的事 AI 全包了，而且浏览器打开不会被反爬拦截，比 AI 自己去抓页面内容靠谱得多。

这篇就来拆解这份"操作手册"里面到底写了什么——不只讲它长什么样，更想聊聊每个设计决策背后在解决什么实际问题。
