...
回顾前面的内容，我们讲了压缩与替换、多层级记忆、子智能体、按需加载……这些处理函数 $F$ 的实现方式，本质上都是**开发者预先设计好的规则**。人类编写好、写死了固定的指令，然后让程序按照这些写死的指令来处理 context。

但如果我们追问一句：**那有没有办法让 Context Engineering 做得智能呢？**
