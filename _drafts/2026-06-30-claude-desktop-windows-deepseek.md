...
**Cowork** 像是给 AI 单独辟出一间虚拟房间，它在里面自己捣鼓文件和浏览器，跟你的真实电脑没有关系；

**Code** 则直接站到你的电脑跟前，能读写本地文件、执行命令。写代码、做 PPT、整理文件这些它俩都能干，区别只是你想让 AI 在你电脑上动手，还是在自己的房间里跑。

> 💡 想用 Cowork，得先给 Windows 开启 [Virtual Machine Platform](https://support.microsoft.com/en-us/windows/enable-virtualization-on-windows-c5578302-6e43-4b4b-a449-8ced115f58e1) 这个虚拟化功能，用 PowerShell 执行一下就行：
>
> ```powershell
> Enable-WindowsOptionalFeature -Online -FeatureName VirtualMachinePlatform -All -NoRestart
> ```
>
> 第一次打开 Code 标签则需要装 Git for Windows，没装的话会提示你，装完重启一下 Claude 就行。
>
...
- 模型列表：填 `claude-opus`、`claude-sonnet` 和 `claude-haiku`

> DeepSeek 会按你传入的 Claude 模型名自动映射：`claude-opus` 开头的模型映射到 `deepseek-v4-pro`，`claude-haiku`、`claude-sonnet` 开头的映射到 `deepseek-v4-flash`。
