...
注意到了吗？👀 **`system`（系统提示）被单独拎了出来**，不和用户对话混在一起。为什么要这么设计？因为系统提示代表开发者对模型行为的约束，它跟用户输入性质完全不同，理应区分对待。这就是 Anthropic 一贯的风格——把安全和可控刻进了 API 设计里。
...
- **Anthropic** 则从第一天就选择了不同的路——把安全刻进接口设计里，哪怕这意味着放弃兼容 OpenAI 格式的便利。
